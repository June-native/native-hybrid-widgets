...
contract NativeLPToken is ERC20, Ownable2Step, ReentrancyGuardTransient {
  ...
  function setEarlyWithdrawFeeBips(
    ...
    require(
      _earlyWithdrawFeeBips <= MAX_EARLY_WITHDRAW_FEE_BIPS,
      ErrorsLib.InvalidFeeBips()
    );
  ...
  /// @dev Can only be called by the owner
  function setRedeemCooldownExempt(
    address account,
    bool status
  ) external onlyOwner {
  ...
  function _validateTransferCooldown(address user) internal view {
    ...
    require(
      lastDepositTimestamp[user] + minRedeemInterval <= block.timestamp ||
        redeemCooldownExempt[user],
